...
const UserUpdate = () => {
  ...
  const handleSearch = async () => {
    try {
      const res = await axios.get(`http://3.226.219.194:5000/api/patients/${mobileNo}`);
      setPatient(res.data);
      setSuccessMessage('');
    } catch (err) {
  ...
  const sendSMSNotification = async (phone) => {
    try {
      const res = await axios.post('http://3.226.219.194:5000/api/send-sms/add-members', {
        phone,
  ...
  const saveToDatabase = async (payment_mode, amount) => {
    try {
      ...
      const res = await axios.post('http://3.226.219.194:5000/api/patient/add-others', {
        mobile_no: mobileNo,
        names,
        total_amount: amount,
        payment_mode,
