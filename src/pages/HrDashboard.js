...
const HRDashboard = () => {
  ...
  const handleToggleCommissions = async () => {
    ...
    try {
      const res = await axios.get(`http://3.226.219.194:5000/api/patients/referred-by/${hr.name}`);
  ...
  const handleSearchPatient = async () => {
    ...
    setSearchMessage('');
    try {
      const res = await axios.get(`http://3.226.219.194:5000/api/patient/by-mobile/${mobileNo}`);
