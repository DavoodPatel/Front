...
function ViewHRs() {
  ...
  const fetchHrList = async () => {
    try {
      const res = await axios.get('http://3.226.219.194:5000/api/hr');
      setHrList(res.data);
    } catch {
  ...
  const toggleReferredPatients = async (hrId, hrName) => {
    ...
    try {
      const res = await axios.get(`http://3.226.219.194:5000/api/patients/referred-by/${hrName}`);
  ...
  const handleDeleteHr = async (id) => {
    ...
    try {
      await axios.delete(`http://3.226.219.194:5000/api/hr/${id}`);
